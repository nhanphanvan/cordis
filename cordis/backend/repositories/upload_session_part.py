...
class UploadSessionPartRepository(BaseRepository[UploadSessionPart]):
    ...
    async def get_for_session_and_part_number(self, *, session_id: str, part_number: int) -> UploadSessionPart | None:
    ...
    async def list_for_session(self, session_id: str) -> list[UploadSessionPart]:
        result = await self.session.execute(
